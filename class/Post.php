...
class Post {	
	...
	public function getPost(){		
		if($this->id) {
			$sqlQuery = "
				SELECT p.id, p.title, p.message, p.rate, p.status, p.created, p.updated 
				FROM ".$this->postTable." p
	...
	public function insert(){
		...
		if($this->title && $this->message) {
			...
			$stmt = $this->conn->prepare("
				INSERT INTO ".$this->postTable."(`title`, `message`, `category_id`, `userid`,`filename`, `status`, `created` , `updated`)
				VALUES(?,?,?,?,?,?,?,?)");
			...
			$stmt->bind_param("ssiissss", $this->title, $this->message, $this->category, $this->userid, $this->filename,$this->status, $this->created, $this->updated);
	...
	public function update(){
		...
		if($this->id) {			
			$stmt = $this->conn->prepare("
				...
				SET title= ?, message = ?, category_id = ?,filename = ?, status= ?, updated = ?
				WHERE id = ?");
			...
			$stmt->bind_param("ssisssi", $this->title, $this->message, $this->category, $this->filename,$this->status, $this->updated, $this->id);
