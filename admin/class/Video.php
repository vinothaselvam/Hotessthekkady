...
class Video {	
	...
	public function getPostsListing(){		
		...
		while ($post = $result->fetch_assoc()) { 				
			...
			$rows[] = $post['created'];	
			$rows[] = $post['updated'];
			$rows[] = '<a href="composevideo.php?id='.$post["id"].'" class="btn btn-warning btn-xs update">Edit</a>';
	...
	public function getPost(){		
		if($this->id) {
			$sqlQuery = "
				SELECT p.id, p.title, p.message, p.category_id, p.status, p.created, p.title_url,p.updated, c.name 
				FROM ".$this->postTable." p
				LEFT JOIN ".$this->categoryTable." c ON c.id = p.category_id
				WHERE p.id = ? ";
			$stmt = $this->conn->prepare($sqlQuery);
	...
	public function insert(){
		...
		if($this->title && $this->message) {
			...
			$stmt = $this->conn->prepare("
				INSERT INTO ".$this->postTable."(`title`, `message`, `category_id`, `userid`,`title_url`, `status`, `created` , `updated`)
				VALUES(?,?,?,?,?,?,?,?)");
			...
			$stmt->bind_param("ssiissss", $this->title, $this->message, $this->category, $this->userid, $this->title_url,$this->status, $this->created, $this->updated);
	...
	public function update(){
		...
		if($this->id) {			
			$stmt = $this->conn->prepare("
				...
				SET title= ?, message = ?, category_id = ?,title_url = ?, status= ?, updated = ?
				WHERE id = ?");
			...
			$stmt->bind_param("ssisssi", $this->title, $this->message, $this->category, $this->title_url,$this->status, $this->updated, $this->id);
