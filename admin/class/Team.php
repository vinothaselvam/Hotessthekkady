...
class Team {	
	...
	public function getPostsListing(){		
		...
		// $sqlQuery = "
		// 	SELECT p.id, p.title,p.message, u.first_name, u.last_name, p.status, p.created, p.updated, 
		...
		while ($post = $result->fetch_assoc()) { 				
			...
			$rows[] = ucfirst($post['message']);		
			$rows[] = ucfirst($post['first_name'])." ".$post['last_name'];	
			$rows[] = $status;				
			$rows[] = $post['created'];	
			$rows[] = $post['updated'];
			$rows[] = '<a href="add_team.php?id='.$post["id"].'" class="btn btn-warning btn-xs update">Edit</a>';
	...
	public function getPost(){		
		if($this->id) {
			$sqlQuery = "
				SELECT p.id, p.title, p.message, p.status, p.created, p.updated
				FROM ".$this->postTable." p
	...
	public function insert(){
		...
		if($this->title && $this->message) {
			...
			$stmt = $this->conn->prepare("
				INSERT INTO ".$this->postTable."(`title`, `message`,  `userid`,`filename`, `status`, `created` , `updated`)
				VALUES(?,?,?,?,?,?,?)");
			...
			$stmt->bind_param("ssissss", $this->title, $this->message,  $this->userid, $this->filename,$this->status, $this->created, $this->updated);
	...
	public function update(){
		...
		if($this->id) {			
			$stmt = $this->conn->prepare("
				...
				SET title= ?, message = ?, filename = ?, status= ?, updated = ?
				WHERE id = ?");
			...
			$stmt->bind_param("sssssi", $this->title, $this->message, $this->filename,$this->status, $this->updated, $this->id);
