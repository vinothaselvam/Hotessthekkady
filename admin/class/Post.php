...
class Post {	
	...
	public function getPostsListing(){		
		...
		$sqlQuery = "
			SELECT p.id, p.title, p.rate, u.first_name, u.last_name, p.status, p.created, p.updated
	...
	public function getPost(){		
		if($this->id) {
			$sqlQuery = "
				SELECT p.id, p.title, p.message, p.rate, p.size,p.capacity,p.bed,p.services,p.status, p.created, p.updated
				FROM ".$this->postTable." p
	...
	public function insert(){
		...
		if($this->title && $this->message) {
			...
			$stmt = $this->conn->prepare("
				INSERT INTO ".$this->postTable."(`title`, `message`, `rate`, `size`, `capacity`, `bed`, `services`, `userid`,`filename`, `status`, `created` , `updated`)
				VALUES(?,?,?,?,?,?,?,?,?,?,?,?)");
			...
			$this->bed = htmlspecialchars(strip_tags($this->bed));
			...
			$stmt->bind_param("sssiississss", $this->title, $this->message, $this->rate,$this->size,$this->capacity,$this->bed,$this->services, $this->userid, $this->filename,$this->status, $this->created, $this->updated);
	...
	public function update(){
		...
		if($this->id) {			
			$stmt = $this->conn->prepare("
				...
				SET title= ?, message = ?, rate = ?, size = ?, capacity = ?,bed = ?, services= ?,filename = ?, status= ?, updated = ?
				WHERE id = ?");
			...
			$stmt->bind_param("sssiisssssi", $this->title, $this->message,$this->rate,$this->size,$this->capacity,$this->bed,$this->services, $this->filename,$this->status, $this->updated, $this->id);
